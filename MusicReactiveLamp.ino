...
#define LONG_SECTOR 20//How many previous sensor values decides if we are on a peak/HIGH (e.g. in a song)
...
void loop() {
  MIC_HIGH = map(analogRead(A1), 0, 1024, 0, 250);
  switch (mode) {
    case 0:
      visualize_music();
      break;
  ...
  delayMicroseconds(DELAY);       // delay in between reads for stability
}
